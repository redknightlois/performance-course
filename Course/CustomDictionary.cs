...
namespace Course
{
    ...
    {
        private readonly Dictionary<long, long> _dictionary = new Dictionary<long, long>(default(NumericEqualityComparer));
        private readonly FastDictionary<long, long, NumericEqualityComparer> _specificFastDictionary = new FastDictionary<long, long, NumericEqualityComparer>(default(NumericEqualityComparer));
        private readonly FastDictionary<long, long> _comparerFastDictionary = new FastDictionary<long, long>(default(NumericEqualityComparer));
        private readonly long[] knownValues = new long[1000];
        ...
        public void Setup()
        {
            ...
            {
                long value = rnd.Next(3 * knownValues.Length);

                knownValues[i] = value;
                _dictionary[value] = value;
                _specificFastDictionary[value] = value;
                _comparerFastDictionary[value] = value;
            }
            
        }

        [Benchmark]
        public long FastDictionaryIComparer()
        {
            long result = 0;
            var dictionary = _comparerFastDictionary;
            long[] kv = knownValues;

            for (int i = 0; i < kv.Length; i++)
            {
                // Avoid the JIT to remove all the code in cases where it can figure out there are no side effects.
                result += dictionary[kv[i]];
            }
        ...
        [Benchmark]
        public long FastDictionarySpecific()
        {
            long result = 0;
            var dictionary = _specificFastDictionary;
            long[] kv = knownValues;
        ...
        where TComparer : IEqualityComparer<TKey>
    {
        ...
        {
            throw new NotSupportedException("Current implementation has a bug, do not use.");
    ...
    public struct NumericEqualityComparer : IEqualityComparer<long>, IEqualityComparer<int>, IEqualityComparer<ulong>, IEqualityComparer<uint>
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static uint Mix(uint key)
        {
